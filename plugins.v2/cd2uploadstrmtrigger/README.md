...
保存、关闭、测试连接、手动测试和规则增删仍保留；启用开关和保存会先校验当前配置，失败时定位到相关页签；令牌输入使用密码控件，不在页面中硬编码令牌或主机敏感信息。MoviePilot API Key 可留空，系统 `API_TOKEN` 会作为回退凭据。
...
- **CD2 目标目录前缀**：`destPath` 的监控范围，只匹配该目标目录及子目录；支持挂载路径、目标路径和 API 路径归一化，例如 `/CloudNAS/115/影视库`、`/115/影视库`、`/影视库`。只有命中启用监控规则的目标目录才会进入处理流程。
...
```text
CD2:   /115/影视库/电影/a.mkv
115:   /影视库/电影/a.mkv
本地:  /media/MP_movieDB/影视库/电影/a.strm 与对应字幕
```

只有目标监控目录中的删除事件才会按 `delete_sync` 开关同步本地 STRM/字幕。

## 状态页与事件历史

状态页分为“总览 / 事件 / 说明”：

- **总览**：连接和监听模式、任务/队列/处理统计、最近 CD2 事件、生成、元数据、字幕、删除同步、Emby 刷新和错误摘要。设置入口统一放在底部。
...
- **事件详情**：点击事件即可折叠/展开，查看 `id`、`at`、`category`、`level`、`title`、`message`、`source`、`status`、原始 `raw_dest_path`、规范化 `path` 和 `details`。
...
`validate` 返回脱敏的配置校验结果、错误字段和页签定位。启用插件至少需要有效的 CD2 gRPC 地址、CD2 Token、启用的目录规则、MoviePilot 地址，以及配置 API Key 或系统 `API_TOKEN` 之一；API Key 留空本身合法。开启 Emby 刷新时还必须存在可用 Emby 服务，缺少前置条件的可选功能会自动回退关闭。`status` 新增 `event_history`、`push_primary`、`poll_fallback_enabled`，并保留原有统计、`last_trigger`、Emby、字幕和删除字段。CD2 Token 需要“获取传输任务”和“接收推送消息”；下载字幕还需要列出文件、读取文件、查看属性以及对应的 HTTP 下载能力。API 根目录必须填写 Token 的 `RootDirectory`，例如 Token 为 `/115` 就填写 `/115`。
