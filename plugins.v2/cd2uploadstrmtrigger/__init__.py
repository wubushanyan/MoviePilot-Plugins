...
class Cd2UploadStrmTrigger(_PluginBase):
    ...
    plugin_desc = "监听 CloudDrive2 新完成任务，媒体调用 115 网盘 STRM 助手生成增量 STRM，字幕按限速下载到本地。"
    ...
    DEFAULT_SUBTITLE_EXTENSIONS = "srt,ssa,ass,vtt,sub,idx"
            ...
            try:
                ...
                for message in client.push_messages(self._stop_event):
                    ...
                    for task in message.transferTaskStatus.uploadFileStatusChanges:
                        self._observe_task(task, allow_trigger=True)
                    self._wake_event.set()
            except Exception as exc:
